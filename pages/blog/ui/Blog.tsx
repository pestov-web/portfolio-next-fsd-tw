import React from 'react';
import { Button } from '@/shared/ui/Button';
import Link from 'next/link';
import { Icon } from '@iconify/react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/shared/ui/Tabs';
import { BlogPostCard } from '@/entities/blog/ui/BlogPostCard';
import { blogPosts } from '@/entities/blog/model/data';

export const Blog: React.FC = () => {
  return (
    <div className="container px-4 py-12 md:px-6 md:py-24">
      <div className="flex flex-col space-y-8">
        <div className="flex flex-col space-y-4">
          <Button variant="ghost" size="sm" className="w-fit" asChild>
            <Link href="/">
              <Icon
                icon="lucide:arrow-left"
                width="24"
                height="24"
                className="mr-2"
              />
              Назад на главную
            </Link>
          </Button>
          <h1 className="text-4xl font-bold">Блог</h1>
          <p className="text-muted-foreground max-w-[700px]">
            Новости, гайды, и статьи которые показались мне интересными или
            важными.
          </p>
        </div>

        <Tabs defaultValue="all" className="w-full">
          <TabsList className="mb-8">
            <TabsTrigger value="all">Все статьи</TabsTrigger>
            <TabsTrigger value="tutorials">Гайды</TabsTrigger>
            <TabsTrigger value="insights">Новости</TabsTrigger>
            <TabsTrigger value="career">Разработка</TabsTrigger>
          </TabsList>
          <TabsContent value="all" className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {blogPosts.map((post) => (
                <BlogPostCard
                  key={post.slug}
                  {...post}
                  slug={`/blog/${post.slug}`}
                />
              ))}
            </div>
            <div className="flex justify-center">
              <Button variant="outline">Загрузить еще</Button>
            </div>
          </TabsContent>
          <TabsContent value="tutorials" className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {blogPosts
                .filter((post) => post.category === 'Tutorials')
                .map((post) => (
                  <BlogPostCard
                    key={post.slug}
                    {...post}
                    slug={`/blog/${post.slug}`}
                  />
                ))}
            </div>
          </TabsContent>
          <TabsContent value="insights" className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {blogPosts
                .filter((post) => post.category === 'Insights')
                .map((post) => (
                  <BlogPostCard
                    key={post.slug}
                    {...post}
                    slug={`/blog/${post.slug}`}
                  />
                ))}
            </div>
          </TabsContent>
          <TabsContent value="career" className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {blogPosts
                .filter((post) => post.category === 'Career')
                .map((post) => (
                  <BlogPostCard
                    key={post.slug}
                    {...post}
                    slug={`/blog/${post.slug}`}
                  />
                ))}
            </div>
          </TabsContent>
        </Tabs>
      </div>
    </div>
  );
};
